...
class UltrasonicDataset(Dataset):
    ...
    def _load_file_data(self) -> None:
        ...
        if not clean_dir.exists() or not noisy_dir.exists():
            ...
            print(
                f"  Running in empty file mode - implement _load_file_data() for real data."
            )
            return
        ...
        print(
            f"[INFO] Loaded {len(self.clean_signals)} signal pairs from {self.data_path}"
        )

    def _generate_gabor_pulse(
        self, delay: float, amplitude: float = 1.0, bandwidth: float = 0.5
    ) -> np.ndarray:
        """
    ...
    def _generate_pulse_burst(
        ...
        if envelope_type == "gaussian":
            ...
            env_values[taper_mask] = 0.5 * (
                1 + np.cos(np.pi * (np.abs(t_norm[taper_mask]) - (1 - alpha)) / alpha)
            )
            ...
            envelope[mask_rise] = np.exp(
                -(t_rel[mask_rise] + half_duration) / (half_duration / 3)
            )
    ...
    def _generate_clean_signal(self) -> np.ndarray:
        ...
        num_bursts = self.rng.integers(
            self.num_pulse_bursts_range[0], self.num_pulse_bursts_range[1] + 1
        )
        ...
        for i in range(num_bursts):
            ...
            num_cycles = self.rng.integers(
                self.cycles_per_burst_range[0], self.cycles_per_burst_range[1] + 1
            )
    ...
    def _generate_impulse_noise(
        ...
        amplitudes = self.rng.uniform(
            amplitude_range[0], amplitude_range[1], num_spikes
        )
    ...
    def _generate_periodic_noise(
        self, length: int, base_freq: Optional[float] = None, num_harmonics: int = 3
    ) -> np.ndarray:
        """
    ...
    def _generate_chirp_interference(self, length: int) -> np.ndarray:
        """
    ...
    def _add_complex_noise(
        self, signal: np.ndarray, snr_db: float, noise_type: str
    ) -> np.ndarray:
        """
        ...
        if noise_type == "gaussian":
            ...
            selected_types = self.rng.choice(
                available_types, min(num_types, len(available_types)), replace=False
            )

            noise = np.zeros(length, dtype=np.float32)
            weights = self.rng.dirichlet(
                np.ones(len(selected_types)) * 0.5
            ...
            for ntype, weight in zip(selected_types, weights):
                if ntype == "gaussian":
                    ...
                    component = self._generate_impulse_noise(
                        length, density=0.05, amplitude_range=(1.0, 3.0)
                    )
                elif ntype == "periodic":
    ...
    def _apply_augmentation(
        ...
        if self.rng.random() < self.augment_prob:
            ...
            extra_noise = (
                self.rng.standard_normal(len(noisy)).astype(np.float32) * noise_std
            )
...
def create_dataloaders(
    ...
    if mode == "file":
        ...
        print(
            f"[INFO] File mode: {len(train_dataset)} training{aug_str}, {len(val_dataset)} validation samples"
        )
    else:
        train_dataset = UltrasonicDataset(
            mode="synthetic",
