...
Usage:
    uv run python inference_deepsets.py --input noisy.mat --output results/deepsets/
    uv run python inference_deepsets.py --input noisy.mat --checkpoint checkpoints/best_deepsets_pinn.pth
"""
...
from data_deepsets import GRID_SPACING
...
def denoise_grid(
    ...
    print(f"\n[INFO] Denoising {total_patches} patches "
            ...
            for cj in range(half, grid_rows - half):
                ...
                for dc in range(-half, half + 1):
                    for dr in range(-half, half + 1):
                        indices.append(
                            (ci + dc) * grid_rows + (cj + dr))
                indices = np.array(indices)

                # Prepare tensors (batch_size=1)
                sig = torch.from_numpy(
                    normalised_signals[indices]).unsqueeze(0).to(device)
                coord = torch.from_numpy(
                    coords_all[indices]).unsqueeze(0).to(device)
    ...
    no_pred = ~mask
    if no_pred.any():
        print(f"  [WARN] {no_pred.sum()} edge points had no prediction, "
...
def run_inference(
    input_path: str,
    output_path: str,
    checkpoint_path: str = 'checkpoints/best_deepsets_pinn.pth',
    grid_cols: int = 21,
    grid_rows: int = 21,
    target_cols: int = 41,
    target_rows: int = 41,
    ...
    normalised, original, metadata = preprocess_mat_data(
        input_path, grid_cols, grid_rows,
        target_cols, target_rows,
        interpolation_method, target_signal_length)

    # Denoise full grid
    denoised_norm = denoise_grid(
        model, normalised,
        target_cols, target_rows, patch_size, device)
    ...
    if metadata.get('interpolated', False):
        from transformer import interpolate_spatial as interp_sp
        grid_big = reshape_to_grid(
            denoised, target_cols, target_rows)
        grid_small = interp_sp(
            grid_big, grid_cols, grid_rows,
            method=interpolation_method)
        denoised_small = flatten_grid(grid_small)

        out_file_orig = out_dir / f"deepsets_denoised_{timestamp}_original.mat"
        sio.savemat(str(out_file_orig), {
            'x': metadata['time_vector'],
            'y': denoised_small,
            'signal_length': metadata['signal_length'],
            'grid_cols': grid_cols,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', '-i', type=str, required=True,
                        help='Input .mat file')
    parser.add_argument('--output', '-o', type=str, default='results/deepsets',
                        help='Output directory')
    parser.add_argument('--checkpoint', '-c', type=str,
                        default='checkpoints/best_deepsets_pinn.pth',
