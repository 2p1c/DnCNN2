...
def _compute_psd(sig: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        (frequencies, psd) 数组对
    """
    freqs, psd = scipy_signal.welch(sig, fs=SAMPLING_RATE, nperseg=256, noverlap=128)
    return freqs, psd
...
def _extract_energy_features(sig: np.ndarray) -> Dict[str, float]:
    ...
    for (low, high), label in zip(SUB_BANDS, SUB_BAND_LABELS):
        ...
        # 存储能量占比 (百分比)
        ratio = (
            (band_energy / np.sum(power_spectrum) * 100)
...
def _extract_wavenumber_features(sig: np.ndarray) -> Dict[str, float]:
    ...
    concentration = float(
        np.sum(valid_mag[conc_mask] ** 2) / (np.sum(valid_mag**2) + 1e-10) * 100
    )
...
def _plot_validation_figure(
    ...
    ax1.plot(
        time_us, pred, "r--", alpha=0.8, linewidth=0.9, label="Predicted (denoised)"
    )
    ...
    ax2.set_title(
        "Panel 2: Power Spectral Density (PSD)", fontsize=12, fontweight="bold"
    )
    ...
    ax2.axvline(
        x=CENTER_FREQ / 1e3, color="gray", linestyle=":", alpha=0.5, label="Center Freq"
    )
    ...
    for feat in key_features:
        tgt_vals = [
            ...
            if not np.isnan(f["target"].get(feat, float("nan")))
        ]
        pred_vals = [
            f["predicted"][feat]
            for f in all_features
            if not np.isnan(f["predicted"].get(feat, float("nan")))
        ]
    ...
    colors_bar = [
        "#4CAF50" if r >= 80 else "#FFC107" if r >= 60 else "#F44336"
        for r in preservation_rates
    ]
    bars = ax3.bar(
    ...
    ax3.set_title(
        "Panel 3: Feature Preservation (Predicted vs Target)",
        fontsize=12,
        fontweight="bold",
    )
    ax3.set_ylim(0, 110)
    ...
    # 多样本平均能量比
    energy_data = {
        label: {"input": [], "target": [], "predicted": []} for label in SUB_BAND_LABELS
    }
    for f in all_features:
    ...
    ax4.bar(
        x_pos + width, pred_means, width, label="Predicted", color="#F44336", alpha=0.8
    )
    ax4.set_xticks(x_pos)
    ax4.set_xticklabels([f"{l} Hz" for l in SUB_BAND_LABELS], fontsize=9)
    ax4.set_ylabel("Energy Ratio (%)")
    ax4.set_title(
        "Panel 4: Sub-band Energy Distribution", fontsize=12, fontweight="bold"
    )
    ...
    ax5.set_title(
        "Panel 5: Cross-Correlation (Target ↔ Predicted)",
        fontsize=12,
        fontweight="bold",
    )
    ...
    ax6.axhline(
        y=0.8, color="green", linestyle="--", alpha=0.5, label="Good threshold (0.8)"
    )
    ...
    ax7.set_title(
        "Panel 7: Summary Statistics (Averaged over all samples)",
        fontsize=12,
        fontweight="bold",
        pad=20,
    )
...
def _print_report(quality_metrics: Dict) -> None:
    ...
    rows = [
        (
            "Cross-Correlation Peak",
            f"{avg['xcorr_target_pred']:.4f}",
            _assess(avg["xcorr_target_pred"], 0.9, 0.7),
        ),
        (
            "Envelope Correlation",
            f"{avg['envelope_corr_target_pred']:.4f}",
            _assess(avg["envelope_corr_target_pred"], 0.85, 0.65),
        ),
        (
            "Spectral Coherence (avg)",
            f"{avg['avg_coherence_target_pred']:.4f}",
            _assess(avg["avg_coherence_target_pred"], 0.8, 0.6),
        ),
        (
            "Arrival Time Error",
            f"{avg['arrival_time_error_us']:.3f} μs",
            _assess_lower(avg["arrival_time_error_us"], 2.0, 5.0),
        ),
        (
            "Peak Amp Preservation",
            f"{avg['peak_amplitude_preservation']:.1f}%",
            _assess(avg["peak_amplitude_preservation"] / 100, 0.8, 0.6),
        ),
        (
            "RMS Preservation",
            f"{avg['rms_preservation']:.1f}%",
            _assess(avg["rms_preservation"] / 100, 0.8, 0.6),
        ),
        (
            "Dominant Freq Preservation",
            f"{avg['dominant_freq_preservation']:.1f}%",
            _assess(avg["dominant_freq_preservation"] / 100, 0.85, 0.65),
        ),
        (
            "Energy Distribution Match",
            f"{avg['energy_distribution_match']:.1f}%",
...
def run_acoustic_validation(
    ...
    for i in range(n):
        ...
        # 子频带能量分布匹配度 (余弦相似度)
        tgt_energy = np.array(
            [fa["target"].get(f"energy_ratio_{l}", 0) for l in SUB_BAND_LABELS]
        )
        pred_energy = np.array(
            [fa["predicted"].get(f"energy_ratio_{l}", 0) for l in SUB_BAND_LABELS]
        )
        denom_cos = np.linalg.norm(tgt_energy) * np.linalg.norm(pred_energy)
        if denom_cos > 1e-10:
            energy_match_list.append(
                float(np.dot(tgt_energy, pred_energy) / denom_cos) * 100
            )
    ...
    _plot_validation_figure(
        inp_np, tgt_np, pred_np, all_features, quality_metrics, save_path
    )
...
def _plot_inference_validation_figure(
    ...
    colors_bar = [
        "#4CAF50" if 70 <= r <= 130 else "#FFC107" if 50 <= r <= 150 else "#F44336"
    ...
    ax3.set_title(
        "Panel 3: Feature Change After Denoising", fontsize=12, fontweight="bold"
    )
    ...
    ax4.bar(
        x_pos - width / 2, inp_means, width, label="Input", color="#2196F3", alpha=0.8
    )
    ax4.bar(
        x_pos + width / 2,
        den_means,
    ...
    ax5.plot(
        freq_coh / 1e3, coh, "r-", linewidth=1.0, alpha=0.8, label="Input ↔ Denoised"
    )
    ax5.axhline(
        y=0.8, color="green", linestyle="--", alpha=0.5, label="Good threshold (0.8)"
    )
    ...
    ax5.set_title(
        "Panel 5: Spectral Coherence (Input ↔ Denoised)", fontsize=12, fontweight="bold"
    )
    ...
    table_data = [
        ...
        [
            "Cross-Correlation\n(Input ↔ Denoised)",
            f"{avg_metrics['xcorr']:.4f}",
            _assess(avg_metrics["xcorr"], 0.7, 0.4),
        ],
        [
            "Envelope Correlation",
            f"{avg_metrics['envelope_corr']:.4f}",
            _assess(avg_metrics["envelope_corr"], 0.7, 0.4),
        ],
        [
            "Spectral Coherence\n(100-500kHz)",
            f"{avg_metrics['avg_coherence']:.4f}",
        ...
        [
            "Noise Reduction\n(RMS decrease)",
            f"{avg_metrics['noise_reduction_pct']:.1f}%",
...
def _print_inference_report(quality_metrics: Dict) -> None:
    ...
    rows = [
        ("Cross-Correlation", f"{avg['xcorr']:.4f}", _assess(avg["xcorr"], 0.7, 0.4)),
        (
            "Envelope Correlation",
            f"{avg['envelope_corr']:.4f}",
            _assess(avg["envelope_corr"], 0.7, 0.4),
        ),
        (
            "Spectral Coherence (avg)",
            f"{avg['avg_coherence']:.4f}",
        ...
        (
            "Energy Dist Similarity",
            f"{avg['energy_distribution_match']:.1f}%",
            _assess(avg["energy_distribution_match"] / 100, 0.8, 0.6),
        ),
        (
            "Noise Reduction (RMS↓)",
            f"{avg['noise_reduction_pct']:.1f}%",
...
def run_inference_validation(
    ...
    for i in range(n):
        ...
        # 子频带能量匹配 (余弦相似度)
        inp_energy = np.array(
            [fa["input"].get(f"energy_ratio_{l}", 0) for l in SUB_BAND_LABELS]
        )
        den_energy = np.array(
            [fa["denoised"].get(f"energy_ratio_{l}", 0) for l in SUB_BAND_LABELS]
        )
        denom_cos = np.linalg.norm(inp_energy) * np.linalg.norm(den_energy)
        if denom_cos > 1e-10:
            energy_match_list.append(
                float(np.dot(inp_energy, den_energy) / denom_cos) * 100
            )
    ...
    quality_metrics = {
        ...
        "averaged": {
            ...
            "noise_reduction_pct": float(np.mean(noise_reduction_list))
            if noise_reduction_list
            else 0.0,
        },
    }
